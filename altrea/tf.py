...
class Proof:
    ...
    def getblock(self, blockid: int):
        """This procedure returns level, assumption and conclusion information
        about a block id.
        ...
        Exceptions:
            NoSuchBlock: The block identifier does not refer to a block.
        """
        ...
        assumption = self.getstatement(self.blocklist[blockid][1][0])
        self.checkassumption(self.blocklist[blockid][1][0])
        conclusion = self.getstatement(self.blocklist[blockid][1][1])
        return [level, assumption, conclusion]
    

    def lem(self, first: int, second: int, comments: str = ''):
        firstblock = self.getblock(first)
        secondblock = self.getblock(second)
        if firstblock[0] != secondblock[0]:
            raise altrea.exception.NotSameLevel(firstblock[0], secondblock[0])
        if firstblock[1].equals(Not(secondblock[1])):
            raise altrea.exception.NotLemOpposites(firstblock[1], secondblock[1])
        if firstblock[2] != secondblock[2]:
            raise altrea.exception.NotSameStatements(firstblock[2], secondblock[2])
        self.addstatement(self,
                          statement=firstblock[2],
                          rule=self.lem_name,
